...
| Day/Night, hover, click, drag, Preview | Passed locally | Story spreads and the animated storm carry authored hover, focus, click, motion, and reveal contracts; the illustrated Atlas and science panoramas retain distinct Day/Night treatment. |
...
| Local code quality gates | Passed | [`app/qa/RELEASE_GATES_2026-08-27.md`](../app/qa/RELEASE_GATES_2026-08-27.md) records the current typecheck, 39/39 Vitest tests, production build, 7/7 Sites tests, and in-app-browser smoke. |
| Runtime cutout asset gate | Blocked | The current alpha/padding audit passes 16 assets and rejects 66 still-referenced v2 cutouts because their subjects touch the image edge. These assets require regeneration or reviewed padding repair before release. |
...
| Real ChatGPT WebMCP host run | Partially verified | The local production bundle registered and exposed exactly six tools in the selected in-app browser, with no registration failure. A recorded host-driven create/compose/patch/presentation/exact-undo execution sequence remains required after the user approves deployment. |
...
| Public source repository | Missing destination | The local checkout has no Git remote. The visible MIT license and challenge-period Git history are present; the current private tree passes typecheck, 39 tests, production build, and 7 Sites tests, while the cutout asset gate remains blocked. Publishing the preserved history and then verifying a fresh anonymous clone remain required. |
