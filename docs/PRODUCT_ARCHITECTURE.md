...
8. `request_image_handoff` — store finals passed inline as base64 data URLs under an explicit `source-photo` or `book-art` role (a 2×2 sheet is split into four tiles in reading order) and return their ids at once; without `images`, open the focused import drawer with the Agent's reason and resolve after the reader provides browser-local assets or dismisses the request.
