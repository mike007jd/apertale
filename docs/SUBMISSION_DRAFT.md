...
The anonymous share reader registers a ninth, separate surface: one read-only tool, `get_shared_book_context`, so a reader who opens a share link with their own ChatGPT can ask about the book they are looking at. It reads the outline, the visible spread's text, and each element's knowledge reveal, declares `readOnlyHint`, and never mutates anything; the eight authoring tools stay on the authoring page only.
