...
| CL-021 | P0 | Production headers | Cloudflare served the static root before the Worker, omitting the WebMCP document policy required by the desktop host. Production verification finding. | FIXED; REDEPLOY PENDING | Ship Cloudflare `_headers` with the static build and rerun the public HTTP verifier. |
