...
| CL-021 | P0 | Production headers | Cloudflare served the static root before the Worker, omitting the WebMCP document policy required by the desktop host. Production verification finding. | FIXED; REDEPLOY PENDING | Exclude `index.html` from static matches, serve an internal app shell through the Worker, and rerun the public HTTP verifier. |
