...
export function buildCreationBrief(input: CreationBriefInput): CreationBrief {
  ...
  const prompt = [
    ...
    "",
    "Phase 2 — after I approve the sketch, continue through generation, import, and creation:",
    "Follow the already-read authoring-guide art and handoff-create hard gates for concurrent generation, bounded import batches, and reuse of verified results. Compose for the approximately 1.62:1 stage. Continue without another confirmation; read only newly reported red marks or conflicts. Perform the image tool's required reference reads and let the browser handle pixel checks.",
    ...
    'Follow the guide\'s present hard gate: set_presentation(surface: "shelf") for the cover and set_presentation(surface: "reader", spreadId) once in the current theme, then inspect for material reading failures. Keep full critique for explicitly requested polish. Repair only failed assets or pages within two rounds. Reuse the same requestId for successful presentation pending; after an ok:false correction use a fresh requestId.',
    "",
    "Completion gates:",
    renderGates(gates),
    "",
