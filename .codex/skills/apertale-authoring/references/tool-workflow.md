...
If a transparent request comes back as an opaque PNG with a baked checkerboard, key the checkerboard out locally with Pillow into a real RGBA matte instead of spending another ImageGen call; regenerate only when the subject itself is wrong. Before importing a cutout, inspect the actual pixels rather than trusting the file extension: the subject must be visible, complete, and padded; the background must be genuinely transparent; the edge must not contain a rectangular matte, chroma spill, detached crop fragments, or a baked glow intended to be supplied by the runtime hover effect. Reject and regenerate failed output.
