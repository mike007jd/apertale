...
- Missing asset: try the host's media transfer. If it is unavailable, call `request_image_handoff` with the correct `assetUse`, use the available Agent capability or open the actual folder for one drag, then refresh asset context.
