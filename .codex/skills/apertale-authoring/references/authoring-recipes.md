...
6. Only after the complete art set exists, transfer generated finals through the supported host path with `assetUse: "book-art"`, then create the book through Site Tools. When direct transfer is unavailable, call `request_image_handoff` and wait for the reader's result.
